...
4. **Роутинг (SPA):**
   - Навигация по URL: `/`, `/about`, `/sermons`, `/events`, `/contact`, `/donation`
   - В workflow деплоя после сборки создаётся `404.html` (копия `index.html`), чтобы прямые ссылки на страницы (например, `/about`) открывались корректно на GitHub Pages.
